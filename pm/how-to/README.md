...
Project-specific content lives in `*.spec` (next to code) and under `pm/`
(`pm/milestones/` — each milestone folder contains its own `open/` and `archive/`,
plus `pm/decisions/`, `pm/decision-tasks/`, `pm/ideas/`, `pm/terms/`,
`pm/reports/`, `pm/audits/`).
