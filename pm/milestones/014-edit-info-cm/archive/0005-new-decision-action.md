# 0005 — Action "neue Decision-Datei"

## Goal
Aus dem UI laesst sich eine neue Decision anlegen: System ermittelt die
naechste globale Decision-Nummer, legt `pm/decisions/NNNN-<slug>.md` mit
Template an, oeffnet im Edit-Mode. Bestehende Decisions bleiben
read-only — Supersede laeuft ueber "neue Decision", nicht ueber Edit.

## Notes
- Decisions sind **append-only**. Diese Regel ist im Save-Endpoint
  (0001) schon serverseitig durchgesetzt; die Action hier ist die
  einzige legale Quelle fuer neue Decisions im UI.
- Wenn die neue Decision eine alte ersetzt, gehoert die Zeile
  `Supersedes: NNNN-<slug>` ins Template — bequemer Eingabe-Slot in
  der Form vorsehen.
- Nummerierung global, max+1 inklusive existierender Files.
- Template-Form steht in `pm/how-to/decisions.md`.
- Action gehoert in die Info-Sektion fuer Decisions; **nicht** als
  Edit-Button an alten Decisions.

## Done when
- Button "Neue Decision" sichtbar in der Decisions-Info-Sektion.
- Klick → Inline-Form mit Slug + (optional) "Supersedes: ..."-Feld.
- Submit → Datei `pm/decisions/NNNN-<slug>.md` existiert mit Template,
  korrekter naechster Nummer; UI springt in den Edit-Mode der neuen
  Decision.
- An bestehenden Decisions taucht weiterhin **kein** Edit-Button auf.
- Supersedes-Feld leer → keine Supersedes-Zeile in der Datei.
  Supersedes-Feld gefuellt → Zeile `Supersedes: <wert>` im Header.

## Out of scope
- Automatische Verlinkung der superseded Decision (kein Backref-
  Schreiben in die alte Datei).
- Hardcoded Liste alter Decisions.
- Validierung, dass die genannte Supersedes-Datei existiert.

See: pm/how-to/decisions.md

## Plan
- **Server-Endpoint**: `app/pm.php` bekommt
  `POST ?action=new_decision`.
  - Body: JSON `{ "slug": "...", "supersedes": "..." | "" }`.
  - Slug-Regex wie 0003/0004. Supersedes-Feld optional, freitext
    (Format `NNNN-slug` oder leer). Keine Validierung dass die genannte
    Decision existiert (out-of-scope laut Notes).
  - Naechste Nummer: `scandir("pm/decisions")`, max+1, 4-stellig.
  - Zielpfad: `pm/decisions/<NNNN>-<slug>.md`. Bei Kollision → 409.
  - Template fest verdrahtet, basierend auf
    `pm/how-to/decisions.md`:
    ```
    # NNNN — <slug-titlecased>

    [Supersedes: <wert>]              # nur wenn supersedes != ""

    - 
    ```
  - Atomar tmp+rename.
  - Antwort: `{ok:true, path, number:NNNN}`.
  - Append-only-Garantie: dieser Endpoint **erstellt** nur neue
    Dateien — Save-Endpoint (014/0001) blockt das Ueberschreiben
    bestehender Decisions. Damit ist die Regel "nie editieren" hart.
- **Sidebar-UI**: `info.php` bekommt im
  `pm/decisions`-Details-Block einen `.btn-new-decision`-Button und
  ein `.new-decision-form` (Slug-Input + optionales Supersedes-Input).
  An vorhandenen Decision-Links **kein** Edit-Button (sichergestellt
  durch 0002).
- **Sidebar-JS** in `plan_loader.js`: analog 0003/0004.
- **Files touched**: `app/pm.php`, `app/info.php`,
  `app/_share/js/plan_loader.js`.

## Verifikation
- `php -l app/pm.php app/info.php` clean.
- Server `php -S 127.0.0.1:8086 -t app` run_in_background.
- Browser auf `info.php`:
  - Decisions-Sektion → Button "+ Decision" sichtbar.
  - Submit Slug `plan-smoke` + Supersedes leer → Datei
    `pm/decisions/<NNNN>-plan-smoke.md` ohne `Supersedes:`-Zeile.
  - Submit Slug `plan-smoke-2` + Supersedes `0007-editor-vendoring` →
    Datei enthaelt `Supersedes: 0007-editor-vendoring` im Header.
  - In beiden Faellen oeffnet sich die neue Decision im Edit-Mode.
  - Cleanup: `rm pm/decisions/<NNNN>-plan-smoke*.md`.
- Bei `ls pm/decisions/` heute (0001-0007) bekommt die naechste
  Decision die Nummer 0008.
- An bestehenden Decisions (z.B. 0007) erscheint **kein** Edit-Button
  (Bestaetigung dass 0002 die Decision-Guard hart durchsetzt).
- `git status` clean, keine `*.tmp.*`.

## Out of scope (Plan)
- Backref-Schreiben in die superseded Decision.
- Validierung dass `supersedes` existiert.

## Done
- `app/pm.php` um POST-Handler `?action=new_decision` erweitert.
  - Body JSON `{slug, supersedes}`. Slug-Whitelist
    `^[a-z0-9][a-z0-9-]*$`, Laenge 1-80. Supersedes optional, Freitext,
    keine Validierung dass die referenzierte Decision existiert (Ticket-
    Notes: out of scope). Newlines im Supersedes -> 400, damit der Header
    eine Zeile bleibt. Ungueltige Slugs/Supersedes -> 400 + Log via
    `app::error_log()`.
  - Naechste Nummer GLOBAL: `scandir(pm/decisions/)`, alle `NNNN-*.md`-
    Eintraege gematcht, max+1. Bei leerem Verzeichnis -> 1. Wird via
    `sprintf("%04d", ...)` zero-padded — analog zu `new_report`.
  - Zielpfad `pm/decisions/NNNN-<slug>.md`. Defensive Collision-Pruefung
    -> 409 (sollte durch max+1 nie greifen).
  - Template fest verdrahtet via HEREDOC, basierend auf
    `pm/how-to/decisions.md` Shape-Block. Title aus Slug abgeleitet
    (`ucwords(str_replace("-", " ", $slug))`). Supersedes-Zeile NUR
    wenn Wert non-empty — sonst startet die Datei nicht mit leeren
    Header-Zeilen. Leere Bullet-Zeile (`- `) ist bewusst — Decisions
    sind "one sentence per line", der User editiert direkt im
    Anschluss im Edit-Mode (M014/0002).
  - Atomar geschrieben via `tmp + rename`. Tmp bei Fehler aufgeraeumt.
  - Antwort 200 + `{ok:true, path, number}`.
  - Append-only ist hart: Save-Endpoint (M014/0001) blockt
    Ueberschreiben bestehender Decisions mit 409. Edit-Button taucht
    bei `pm/decisions/`-Pfaden gar nicht erst auf (M014/0002). Dieser
    Endpoint hier kann nur create.
  - Spec-Block ueber dem Handler dokumentiert Vertrag + Append-only-
    Garantie; Datei-Header-Spec um den neuen Endpoint erweitert.
- `app/info.php`: dritter Action-Block analog zum Reports-Block in
  M014/0004, fuer die Decisions-Sektion. Form enthaelt `.input-slug`
  (Slug-Input, maxlength 80) plus `.input-supersedes` (optionaler
  Freitext-Input, maxlength 120). Klassen `.btn-new-decision`,
  `.new-decision-form`, `.btn-cancel-form`, `.form-error` — spiegeln
  die existierenden Patterns. An vorhandenen Decision-Links erscheint
  weiterhin KEIN Edit-Button (sichergestellt durch M014/0002, keine
  Aenderung an dieser Stelle noetig).
- `app/_share/js/plan_loader.js` um `init_new_decision_form()` plus die
  Handler `on_new_decision_button_click`, `on_new_decision_cancel_click`,
  `on_new_decision_submit` erweitert. Submit POSTet
  `{slug, supersedes}` an `/pm.php?action=new_decision`; Cancel
  resettet beide Inputs. Bei `ok:true` navigiert auf
  `info.php?path=<data.path>` — der Server liefert den vollen Pfad
  inkl. NNNN-Prefix, Client baut ihn nicht selbst zusammen.
  Self-guarded (no-op ohne Button/Form im DOM, z.B. auf plan.php).
  Spec-Block oben um den M014/0005-Abschnitt erweitert.

Files touched:
- `app/pm.php` (+~270 / -2: Header-Spec, action-Flag, Handler-Block
  mit Spec-Kommentar).
- `app/info.php` (+~25: dritter Action-Block fuer Decisions + Aufruf
  von render_info_section mit dem 4. Argument).
- `app/_share/js/plan_loader.js` (+~235: Spec-Block-Erweiterung,
  init_new_decision_form-Funktionsfamilie, Aufruf in init_plan_loader).
- `pm/milestones/014-edit-info-cm/milestone.md` (Haekchen +
  archive/-Pfad fuer 0005).
- Ticket selbst nach `archive/`.

Smoketest-Belege (Server `php -S 127.0.0.1:8086 -t app`):
- `php -l app/pm.php app/info.php` -> clean.
- Pre-State `ls pm/decisions/` -> 0001-0007. Naechste = 0008.
- Happy ohne Supersedes
  `{"slug":"agent-smoke","supersedes":""}` -> 200 +
  `{ok:true,path:"pm/decisions/0008-agent-smoke.md",number:"0008"}`.
  `cat` zeigt `# 0008 — Agent Smoke` plus leere Bullet, KEINE
  Supersedes-Zeile. Cleanup ok.
- Happy mit Supersedes
  `{"slug":"agent-smoke","supersedes":"0007-editor-vendoring"}` -> 200
  + 0008. `cat` zeigt `Supersedes: 0007-editor-vendoring`-Zeile. Vor
  Cleanup zweiter POST `save&path=pm/decisions/0008-agent-smoke.md` ->
  409 + `Decision ist append-only.` (M014/0001 greift). Cleanup ok.
- Reject bad slug `{"slug":"Bad!","supersedes":""}` -> 400 +
  `Ungueltiger slug.`.
- Reject empty `{}` -> 400 + `Ungueltiger slug.`.
- Reject traversal `{"slug":"../etc","supersedes":""}` -> 400 +
  `Ungueltiger slug.`.
- Reject leading dash `{"slug":"-bad","supersedes":""}` -> 400.
- Reject newline supersedes
  `{"slug":"agent-smoke","supersedes":"line1\nline2"}` -> 400 +
  `Ungueltiges supersedes.`.
- HTML `curl info.php | grep -c btn-new-decision` -> 1.
- HTML `curl info.php | grep -c new-decision-form` -> 1.
- JS `grep -n "new-decision\|new_decision" plan_loader.js` -> 27
  Matches (Spec-Block + Selektoren + Submit-Body + init-Aufruf).
- Regression M014/0003 `{"slug":"regr-smoke"}` an `?action=new_idea`
  -> 200 + `{ok:true,path:"pm/ideas/regr-smoke.md"}`. Cleanup ok.
- Regression M014/0004
  `{"slug":"regr-smoke","type":"research"}` an `?action=new_report`
  -> 200 + 0003. Cleanup ok.
- Streu-Files: `*.tmp.*` leer, `app.sqlite*` zeigt nur `./app.sqlite`.
- Server via `TaskStop` gestoppt; Port 8083 nicht angefasst.
